...
def ensure_books_file(BOOKS_FILE):
    if not os.path.exists(BOOKS_FILE):
        ...
        default_books = {"systemBooks": [], "userBooks": []}
...
def chat_req(conversation_id=None):
    ...
    try:
        ...
        if path == "vector" and book_name == "":
            reply = (
                "StudyMate Error : Please set a valid Book name , using the side bar "
            )
            print("Sending:", reply)
...
def book_name_get(conversation_id=None):
    ...
    if book_name:
        ...
        return jsonify({"book_name": f"{book_name}", "bookName": book_name})
    if not book_name:
...
def import_file(conversation_id=None):
    ...
    if book_name in system_books:
        return (
            jsonify({"message": "You cannot update system books", "status": "error"}),
            400,
        )
...
Starting Background processing..."""
        ...
        save_mail("System", f"Error uploading file: {str(e)}", mail_file_path=mail_file)
        return (
            jsonify(
                {
...
def get_books_list(conversation_id=None):
    ...
    try:
        ...
        return (
            jsonify(
                {"success": False, "message": "Failed to fetch books", "error": str(e)}
            ),
            500,
        )
...
def add_book(conversation_id=None):
    ...
    try:
        ...
        if book_exists:
            return jsonify({"success": False, "message": "Book already exists"}), 409
        ...
        if write_books(books, book_file):
            ...
            return jsonify({"success": False, "message": "Failed to save book"}), 500

    except Exception as e:
        print(f"Error adding book: {e}")
        return (
            jsonify(
                {"success": False, "message": "Failed to add book", "error": str(e)}
            ),
            500,
        )
...
def delete_book(conversation_id=None):
    ...
    try:
        ...
        is_system_book = any(
            book.lower() == book_name.lower() for book in books.get("systemBooks", [])
        )

        if is_system_book:
            return (
                jsonify({"success": False, "message": "Cannot delete system books"}),
                403,
            )
        ...
        if book_in_file:
            ...
            if write_books(books, book_file):
                ...
                return (
                    jsonify({"success": False, "message": "Failed to delete book"}),
                    500,
                )
...
def get_system_books(conversation_id=None):
    ...
    try:
        books = read_books(book_file)
        return jsonify({"success": True, "books": books.get("systemBooks", [])})
    except Exception as e:
...
def get_user_books(conversation_id=None):
    ...
    try:
        books = read_books(book_file)
        return jsonify({"success": True, "books": books.get("userBooks", [])})
    except Exception as e:
